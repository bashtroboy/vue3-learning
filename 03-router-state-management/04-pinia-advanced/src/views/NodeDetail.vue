<template>
  <div class="node-detail">
    ...
    <div v-if="loading && !currentNode" class="loading-state">
      <div class="spinner"></div>
      <p>Loading node details...</p>
    </div>

    <div v-else-if="error && !currentNode" class="error-state">
...
const { getChildNodes, getBreadcrumbPath } = contentStore
...
const isFavorite = computed(() => {
  ...
  return isFavoriteGetter.value(currentNode.value.id)
})
...
async function toggleFavorite() {
  ...
  try {
    await favoritesStore.toggleFavorite(currentNode.value.id)
  } catch (error) {
    // Error is handled by the store and notification
  }
}

onMounted(() => {
  loadNode(nodeId.value)
})

// Watch for route changes (when navigating between nodes)
watch(() => route.params.id, (newId) => {
  if (newId) {
    loadNode(parseInt(newId))
  }
})
</script>
